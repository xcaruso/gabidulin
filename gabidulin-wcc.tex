...
and $L = \llcm((X - b_{i,j})_{1 \leq i \leq s, \, 1 \leq j \leq \delta_i})$.
...
\begin{comment}
Set $D = R - UP$. From the identity $D = VL + U{\cdot}(\tilde P - P)$,
we deduce that, for $1 \leq i \leq s$ and $1 \leq j 
\leq n_i$, we have $\ev{c_i}(D)(g_{i,j}) = \ev{c_i}(U) (e_{i,j})$
where $e_{i,j}$ denote the $(i,j)$ coordinate of the error vector $e$.
From this, we derive
$\wrH(\gamma_{\bc,\bg}(D)) \leq \wrH(e) \leq w$.
